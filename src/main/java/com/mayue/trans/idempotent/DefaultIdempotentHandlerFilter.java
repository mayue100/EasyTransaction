...
import com.mayue.trans.idempotent.exception.ResultWrapperExeception;
...
import com.mayue.trans.util.ObjectDigestUtil;
			...
			try {
				result = transactionTemplate.execute(new TransactionCallback<EasyTransResult>() {
							...
							public EasyTransResult doInTransaction(TransactionStatus status) {
								...
								IdempotentHelper.IdempotentPo idempotentPo = helper.getIdempotentPo(filterChain, request);
								...
								if(idempotentPo == null){
									idempotentPo = new IdempotentHelper.IdempotentPo();
							...
							public EasyTransResult filterResult(IdempotentHelper.IdempotentPo idempotentPo, String objectMD5, ExecuteOrder executeOrder){
